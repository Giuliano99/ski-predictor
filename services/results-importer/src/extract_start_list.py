...
def parse_group(line: str) -> dict[str, Any] | None:
    ...
    if not age_match or not re.search(r"weiblich|männlich|maennlich|Mädchen|Maedchen|Buben", line, re.IGNORECASE):
        return None

    age_class = age_match.group(1).upper()
    category = "FEMALE" if re.search(r"weiblich|Mädchen|Maedchen", line, re.IGNORECASE) else "MALE"
    ...
    return {
        ...
        "label": clean_space(re.sub(r"\s*\|\s*\d+\s+Starter\s*$", "", line, flags=re.IGNORECASE)),
        "ageClass": age_class,
...
def parse_reconstructed(lines: list[str], target_club: str) -> tuple[list[dict[str, Any]], list[str]]:
    """Parse the fixed-column PDFs generated by the local start-list builder."""
    ...
    while index < len(lines):
        ...
        if line == "Stnr":
            headers: list[str] = []
            index += 1
            while index < len(lines) and len(headers) < 7:
                headers.append(lines[index])
                index += 1
                if headers[-1] == "Laufzeit":
                    break
            table_has_code = "Code" in headers
            in_table = bool(current_group and headers and headers[-1] == "Laufzeit")
            continue

        if not in_table or current_group is None or not line.isdigit():
            index += 1
            continue

        field_count = 5 if table_has_code else 3
        values = lines[index + 1:index + 1 + field_count]
        birth_year_index = 2 if table_has_code else 1
        if len(values) != field_count or not re.fullmatch(r"(?:19|20)\d{2}", values[birth_year_index]):
            warnings.append(f"Nicht erkannte Tabellenzeile in {current_group['label']} ab Startnummer {line}")
            index += 1
            continue

        if table_has_code:
            code, raw_name, birth_year, federation, club = values
        else:
            raw_name, birth_year, club = values
            code = federation = None

        if "," not in raw_name:
            warnings.append(f"Name ohne erwartetes Komma in {current_group['label']}: {raw_name}")
            index += field_count + 1
            continue
        ...
        starter: dict[str, Any] = {
            ...
            "targetClub": is_target_club(normalized_club, target_club),
        }
        if code:
            starter["externalAthleteId"] = code
        if federation:
            starter["federation"] = federation
        current_group["starters"].append(starter)
        index += field_count + 1

    expected = sum(int(value) for value in re.findall(r"\|\s*(\d+)\s+Starter\b", "\n".join(lines), re.IGNORECASE))
    actual = sum(len(group["starters"]) for group in groups)
    if expected and expected != actual:
        warnings.append(f"Starterzahl abweichend: angekündigt {expected}, erkannt {actual}")
